...
usertrap(void)
{
  ...
  if(which_dev == 2){
    p->passed_ticks_cnt++;
    if(p->passed_ticks_cnt == p->interval){
      if(p->inalarm == 0){
      
      savereg(p->trapframe,p->trapframesave);
      p->passed_ticks_cnt = 0;
...
void savereg(struct trapframe *trapframe, struct trapframe *trapframesave){
